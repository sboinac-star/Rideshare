...
function maskIfPhone(name: string): string {
  // Old chats stored raw phone numbers — never display them
  return /^\+?[\d\s\-().]{7,}$/.test(name.trim()) ? `Member ••${name.trim().slice(-4)}` : name;
}
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {chats.map((chat) => {
              ...
              const otherName = maskIfPhone(chat.participantNames[otherUid] ?? "");
              return (
                <button
                  key={chat.id}
      ...
      {openChat && (() => {
        ...
        const otherName = maskIfPhone(openChat.participantNames[otherUid] ?? "");
        return (
          <ChatModal
            chatId={openChat.id}
            ownerUid={otherUid}
