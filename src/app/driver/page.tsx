...
export default function DriverPage() {
  ...
  useEffect(() => {
    ...
    if (new URLSearchParams(window.location.search).get("event") === "1" && isFeaturedEventActive()) {
      setNewJourney((j) => ({ ...j, category: "event", eventName: featuredEvent.shortName }));
    }
  }, []);
            ...
            <form onSubmit={handlePostJourney} className="space-y-4">
              ...
              <div>
                ...
                {newJourney.category === "event" && (
                  <>
                    {isFeaturedEventActive() && newJourney.eventName !== featuredEvent.shortName && (
                      <button
                        type="button"
                        onClick={() => setNewJourney({ ...newJourney, eventName: featuredEvent.shortName })}
