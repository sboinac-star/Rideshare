...
export async function GET(req: Request) {
  ...
  const { ratingSum, ratingCount, cancelCount } = snap.data() as { ratingSum: number; ratingCount: number; cancelCount?: number };
  const rating = ratingCount > 0 ? Math.round((ratingSum / ratingCount) * 10) / 10 : null;
  return Response.json({ rating, count: ratingCount, cancelCount: cancelCount ?? 0 });
}
